$(document).ready(function(){
    ...
    var stock_symbols = getStockSymbols();
    $stock_lookup = $('#stock-lookup');
    autocomplete(document.getElementById("stock-lookup"), stock_symbols);
    ...
    $.ajax({
        ...
        success: function(stocks) {
            $.each(stocks, function(i, stock) {
                stock_symbols.push([stock.symbol, stock.name]);
            })
        }
    });
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr_length; i++) {
          ...
          if (arr[i][0].substring(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            b.innerHTML += arr[i][0].substring(val.length) + ": " + arr[i][1];
